...
# OpenROAD power analysis script for carry_select_wrapper design
# Handles both carry_select_wrapper and carry_select_adder_wrapper naming
...
foreach name {"carry_select_wrapper" "carry_select_adder_wrapper"} {
    if {[file exists "${run_dir}/final/pnl/${name}.pnl.v"]} {
        set design_name $name
        set pnl_file "${run_dir}/final/pnl/${name}.pnl.v"
        break
    } elseif {[file exists "${run_dir}/36-openroad-resizertimingpostcts/${name}.pnl.v"]} {
        set design_name $name
        set pnl_file "${run_dir}/36-openroad-resizertimingpostcts/${name}.pnl.v"
        break
    }
}

if {$design_name eq ""} {
    puts "ERROR: Could not find powered netlist for carry_select design"
    exit 1
}
