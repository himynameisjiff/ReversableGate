...
if ! command -v openroad &> /dev/null; then
    ...
    echo ""
    echo "  OpenLane Nix shell (if you have OpenLane installed):"
    echo "    cd <your-openlane-directory>"
    echo "    nix-shell"
    echo "    # Then run this script from within the nix-shell"
    echo ""
    echo "  Or run this script directly with nix-shell:"
    echo "    nix-shell <openlane-path>/shell.nix --run \"$0\""
    echo ""
    ...
    echo ""
    echo "  Docker:"
    echo "    docker pull openroad/flow-scripts"
    echo "    docker run -it -v \$(pwd):/work openroad/flow-scripts bash"
    echo ""
