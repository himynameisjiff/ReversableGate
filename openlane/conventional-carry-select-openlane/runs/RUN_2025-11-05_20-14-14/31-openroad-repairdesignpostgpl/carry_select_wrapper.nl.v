module carry_select_wrapper (cin,
    clk,
    cout,
    reset,
    a,
    b,
    sum);
 input cin;
 input clk;
 output cout;
 input reset;
 input [7:0] a;
 input [7:0] b;
 output [7:0] sum;

 wire _000_;
 wire _001_;
 wire _002_;
 wire _003_;
 wire _004_;
 wire _005_;
 wire _006_;
 wire _007_;
 wire _008_;
 wire _009_;
 wire _010_;
 wire _011_;
 wire _012_;
 wire _013_;
 wire _014_;
 wire _015_;
 wire _016_;
 wire _017_;
 wire _018_;
 wire _019_;
 wire _020_;
 wire _021_;
 wire _022_;
 wire _023_;
 wire _024_;
 wire _025_;
 wire _026_;
 wire _027_;
 wire _028_;
 wire _029_;
 wire _030_;
 wire _031_;
 wire _032_;
 wire _033_;
 wire _034_;
 wire _035_;
 wire _036_;
 wire _037_;
 wire _038_;
 wire _039_;
 wire _040_;
 wire _041_;
 wire _042_;
 wire _043_;
 wire _044_;
 wire _045_;
 wire _046_;
 wire _047_;
 wire _048_;
 wire _049_;
 wire _050_;
 wire _051_;
 wire _052_;
 wire _053_;
 wire _054_;
 wire _055_;
 wire _056_;
 wire _057_;
 wire _058_;
 wire _059_;
 wire _060_;
 wire _061_;
 wire _062_;
 wire _063_;
 wire _064_;
 wire _065_;
 wire _066_;
 wire _067_;
 wire cin_r;
 wire cout_c;
 wire \sum_c[0] ;
 wire \sum_c[1] ;
 wire \sum_c[2] ;
 wire \sum_c[3] ;
 wire \sum_c[4] ;
 wire \sum_c[5] ;
 wire \sum_c[6] ;
 wire \sum_c[7] ;
 wire \u_cs.hi_block_c0.fa0.a ;
 wire \u_cs.hi_block_c0.fa0.b ;
 wire \u_cs.hi_block_c0.fa1.a ;
 wire \u_cs.hi_block_c0.fa1.b ;
 wire \u_cs.hi_block_c0.fa2.a ;
 wire \u_cs.hi_block_c0.fa2.b ;
 wire \u_cs.hi_block_c0.fa3.a ;
 wire \u_cs.hi_block_c0.fa3.b ;
 wire \u_cs.lo_block.fa0.a ;
 wire \u_cs.lo_block.fa0.b ;
 wire \u_cs.lo_block.fa1.a ;
 wire \u_cs.lo_block.fa1.b ;
 wire \u_cs.lo_block.fa2.a ;
 wire \u_cs.lo_block.fa2.b ;
 wire \u_cs.lo_block.fa3.a ;
 wire \u_cs.lo_block.fa3.b ;
 wire net1;
 wire net2;
 wire net3;
 wire net4;
 wire net5;
 wire net6;
 wire net7;
 wire net8;
 wire net9;
 wire net10;
 wire net11;
 wire net12;
 wire net13;
 wire net14;
 wire net15;
 wire net16;
 wire net17;
 wire net18;
 wire net19;
 wire net20;
 wire net21;
 wire net22;
 wire net23;
 wire net24;
 wire net25;
 wire net26;
 wire net27;
 wire net28;
 wire net29;
 wire net30;

 sky130_fd_sc_hd__inv_2 _068_ (.A(net28),
    .Y(_000_));
 sky130_fd_sc_hd__nand2_1 _069_ (.A(\u_cs.lo_block.fa0.a ),
    .B(cin_r),
    .Y(_028_));
 sky130_fd_sc_hd__or2_1 _070_ (.A(\u_cs.lo_block.fa0.a ),
    .B(cin_r),
    .X(_029_));
 sky130_fd_sc_hd__and3_1 _071_ (.A(\u_cs.lo_block.fa0.b ),
    .B(_028_),
    .C(_029_),
    .X(_030_));
 sky130_fd_sc_hd__a21boi_1 _072_ (.A1(\u_cs.lo_block.fa0.b ),
    .A2(_029_),
    .B1_N(_028_),
    .Y(_031_));
 sky130_fd_sc_hd__and2b_1 _073_ (.A_N(_031_),
    .B(\u_cs.lo_block.fa1.a ),
    .X(_032_));
 sky130_fd_sc_hd__xnor2_1 _074_ (.A(\u_cs.lo_block.fa1.a ),
    .B(_031_),
    .Y(_033_));
 sky130_fd_sc_hd__a21oi_1 _075_ (.A1(\u_cs.lo_block.fa1.b ),
    .A2(_033_),
    .B1(_032_),
    .Y(_034_));
 sky130_fd_sc_hd__and2b_1 _076_ (.A_N(_034_),
    .B(\u_cs.lo_block.fa2.a ),
    .X(_035_));
 sky130_fd_sc_hd__inv_2 _077_ (.A(_035_),
    .Y(_036_));
 sky130_fd_sc_hd__xnor2_1 _078_ (.A(\u_cs.lo_block.fa2.a ),
    .B(_034_),
    .Y(_037_));
 sky130_fd_sc_hd__nand2_1 _079_ (.A(\u_cs.lo_block.fa2.b ),
    .B(_037_),
    .Y(_038_));
 sky130_fd_sc_hd__a21boi_1 _080_ (.A1(_036_),
    .A2(_038_),
    .B1_N(\u_cs.lo_block.fa3.a ),
    .Y(_039_));
 sky130_fd_sc_hd__or3b_1 _081_ (.A(\u_cs.lo_block.fa3.a ),
    .B(_035_),
    .C_N(_038_),
    .X(_040_));
 sky130_fd_sc_hd__and2b_1 _082_ (.A_N(_039_),
    .B(_040_),
    .X(_041_));
 sky130_fd_sc_hd__a21oi_2 _083_ (.A1(\u_cs.lo_block.fa3.b ),
    .A2(_040_),
    .B1(_039_),
    .Y(_042_));
 sky130_fd_sc_hd__xnor2_2 _084_ (.A(\u_cs.hi_block_c0.fa3.a ),
    .B(\u_cs.hi_block_c0.fa3.b ),
    .Y(_043_));
 sky130_fd_sc_hd__nand2_1 _085_ (.A(\u_cs.hi_block_c0.fa2.a ),
    .B(\u_cs.hi_block_c0.fa2.b ),
    .Y(_044_));
 sky130_fd_sc_hd__or2_1 _086_ (.A(\u_cs.hi_block_c0.fa2.a ),
    .B(\u_cs.hi_block_c0.fa2.b ),
    .X(_045_));
 sky130_fd_sc_hd__nand2_1 _087_ (.A(_044_),
    .B(_045_),
    .Y(_046_));
 sky130_fd_sc_hd__nor2_1 _088_ (.A(\u_cs.hi_block_c0.fa1.a ),
    .B(\u_cs.hi_block_c0.fa1.b ),
    .Y(_047_));
 sky130_fd_sc_hd__nand2_1 _089_ (.A(\u_cs.hi_block_c0.fa1.a ),
    .B(\u_cs.hi_block_c0.fa1.b ),
    .Y(_048_));
 sky130_fd_sc_hd__nor2_1 _090_ (.A(\u_cs.hi_block_c0.fa0.a ),
    .B(\u_cs.hi_block_c0.fa0.b ),
    .Y(_049_));
 sky130_fd_sc_hd__o21a_1 _091_ (.A1(_047_),
    .A2(_049_),
    .B1(_048_),
    .X(_050_));
 sky130_fd_sc_hd__or2_1 _092_ (.A(_046_),
    .B(_050_),
    .X(_051_));
 sky130_fd_sc_hd__nand2_1 _093_ (.A(_044_),
    .B(_051_),
    .Y(_052_));
 sky130_fd_sc_hd__xnor2_1 _094_ (.A(_043_),
    .B(_052_),
    .Y(_053_));
 sky130_fd_sc_hd__nand2b_1 _095_ (.A_N(_047_),
    .B(_048_),
    .Y(_054_));
 sky130_fd_sc_hd__nand2_1 _096_ (.A(\u_cs.hi_block_c0.fa0.a ),
    .B(\u_cs.hi_block_c0.fa0.b ),
    .Y(_055_));
 sky130_fd_sc_hd__o21a_1 _097_ (.A1(_047_),
    .A2(_055_),
    .B1(_048_),
    .X(_056_));
 sky130_fd_sc_hd__nor2_1 _098_ (.A(_046_),
    .B(_056_),
    .Y(_057_));
 sky130_fd_sc_hd__inv_2 _099_ (.A(_057_),
    .Y(_058_));
 sky130_fd_sc_hd__and3_1 _100_ (.A(_043_),
    .B(_044_),
    .C(_058_),
    .X(_059_));
 sky130_fd_sc_hd__a21oi_1 _101_ (.A1(_044_),
    .A2(_058_),
    .B1(_043_),
    .Y(_060_));
 sky130_fd_sc_hd__nor2_1 _102_ (.A(_059_),
    .B(_060_),
    .Y(_061_));
 sky130_fd_sc_hd__mux2_1 _103_ (.A0(_053_),
    .A1(_061_),
    .S(_042_),
    .X(\sum_c[7] ));
 sky130_fd_sc_hd__nand2_1 _104_ (.A(_046_),
    .B(_056_),
    .Y(_062_));
 sky130_fd_sc_hd__a21oi_1 _105_ (.A1(_046_),
    .A2(_050_),
    .B1(_042_),
    .Y(_063_));
 sky130_fd_sc_hd__a32o_1 _106_ (.A1(_042_),
    .A2(_058_),
    .A3(_062_),
    .B1(_063_),
    .B2(_051_),
    .X(\sum_c[6] ));
 sky130_fd_sc_hd__and2b_1 _107_ (.A_N(_049_),
    .B(_055_),
    .X(_064_));
 sky130_fd_sc_hd__nand2b_1 _108_ (.A_N(_042_),
    .B(_064_),
    .Y(_065_));
 sky130_fd_sc_hd__xor2_1 _109_ (.A(_054_),
    .B(_055_),
    .X(_066_));
 sky130_fd_sc_hd__mux2_1 _110_ (.A0(_054_),
    .A1(_066_),
    .S(_065_),
    .X(\sum_c[5] ));
 sky130_fd_sc_hd__xnor2_1 _111_ (.A(_042_),
    .B(_064_),
    .Y(\sum_c[4] ));
 sky130_fd_sc_hd__nor3b_1 _112_ (.A(_042_),
    .B(_043_),
    .C_N(_052_),
    .Y(_067_));
 sky130_fd_sc_hd__a211o_1 _113_ (.A1(\u_cs.hi_block_c0.fa3.a ),
    .A2(\u_cs.hi_block_c0.fa3.b ),
    .B1(_060_),
    .C1(_067_),
    .X(cout_c));
 sky130_fd_sc_hd__a21oi_1 _114_ (.A1(_028_),
    .A2(_029_),
    .B1(\u_cs.lo_block.fa0.b ),
    .Y(_026_));
 sky130_fd_sc_hd__nor2_1 _115_ (.A(_030_),
    .B(_026_),
    .Y(\sum_c[0] ));
 sky130_fd_sc_hd__xor2_1 _116_ (.A(\u_cs.lo_block.fa1.b ),
    .B(_033_),
    .X(\sum_c[1] ));
 sky130_fd_sc_hd__or2_1 _117_ (.A(\u_cs.lo_block.fa2.b ),
    .B(_037_),
    .X(_027_));
 sky130_fd_sc_hd__and2_1 _118_ (.A(_038_),
    .B(_027_),
    .X(\sum_c[2] ));
 sky130_fd_sc_hd__xor2_1 _119_ (.A(\u_cs.lo_block.fa3.b ),
    .B(_041_),
    .X(\sum_c[3] ));
 sky130_fd_sc_hd__inv_2 _120_ (.A(net29),
    .Y(_001_));
 sky130_fd_sc_hd__inv_2 _121_ (.A(net29),
    .Y(_002_));
 sky130_fd_sc_hd__inv_2 _122_ (.A(net29),
    .Y(_003_));
 sky130_fd_sc_hd__inv_2 _123_ (.A(net29),
    .Y(_004_));
 sky130_fd_sc_hd__inv_2 _124_ (.A(net29),
    .Y(_005_));
 sky130_fd_sc_hd__inv_2 _125_ (.A(net29),
    .Y(_006_));
 sky130_fd_sc_hd__inv_2 _126_ (.A(net29),
    .Y(_007_));
 sky130_fd_sc_hd__inv_2 _127_ (.A(net28),
    .Y(_008_));
 sky130_fd_sc_hd__inv_2 _128_ (.A(net28),
    .Y(_009_));
 sky130_fd_sc_hd__inv_2 _129_ (.A(net29),
    .Y(_010_));
 sky130_fd_sc_hd__inv_2 _130_ (.A(net30),
    .Y(_011_));
 sky130_fd_sc_hd__inv_2 _131_ (.A(net29),
    .Y(_012_));
 sky130_fd_sc_hd__inv_2 _132_ (.A(net28),
    .Y(_013_));
 sky130_fd_sc_hd__inv_2 _133_ (.A(net28),
    .Y(_014_));
 sky130_fd_sc_hd__inv_2 _134_ (.A(net28),
    .Y(_015_));
 sky130_fd_sc_hd__inv_2 _135_ (.A(net28),
    .Y(_016_));
 sky130_fd_sc_hd__inv_2 _136_ (.A(net30),
    .Y(_017_));
 sky130_fd_sc_hd__inv_2 _137_ (.A(net30),
    .Y(_018_));
 sky130_fd_sc_hd__inv_2 _138_ (.A(net30),
    .Y(_019_));
 sky130_fd_sc_hd__inv_2 _139_ (.A(net29),
    .Y(_020_));
 sky130_fd_sc_hd__inv_2 _140_ (.A(net28),
    .Y(_021_));
 sky130_fd_sc_hd__inv_2 _141_ (.A(net28),
    .Y(_022_));
 sky130_fd_sc_hd__inv_2 _142_ (.A(net28),
    .Y(_023_));
 sky130_fd_sc_hd__inv_2 _143_ (.A(net30),
    .Y(_024_));
 sky130_fd_sc_hd__inv_2 _144_ (.A(net30),
    .Y(_025_));
 sky130_fd_sc_hd__dfrtp_1 _145_ (.CLK(clk),
    .D(\sum_c[0] ),
    .RESET_B(_000_),
    .Q(net20));
 sky130_fd_sc_hd__dfrtp_1 _146_ (.CLK(clk),
    .D(\sum_c[1] ),
    .RESET_B(_001_),
    .Q(net21));
 sky130_fd_sc_hd__dfrtp_1 _147_ (.CLK(clk),
    .D(\sum_c[2] ),
    .RESET_B(_002_),
    .Q(net22));
 sky130_fd_sc_hd__dfrtp_1 _148_ (.CLK(clk),
    .D(\sum_c[3] ),
    .RESET_B(_003_),
    .Q(net23));
 sky130_fd_sc_hd__dfrtp_1 _149_ (.CLK(clk),
    .D(\sum_c[4] ),
    .RESET_B(_004_),
    .Q(net24));
 sky130_fd_sc_hd__dfrtp_1 _150_ (.CLK(clk),
    .D(\sum_c[5] ),
    .RESET_B(_005_),
    .Q(net25));
 sky130_fd_sc_hd__dfrtp_1 _151_ (.CLK(clk),
    .D(\sum_c[6] ),
    .RESET_B(_006_),
    .Q(net26));
 sky130_fd_sc_hd__dfrtp_1 _152_ (.CLK(clk),
    .D(\sum_c[7] ),
    .RESET_B(_007_),
    .Q(net27));
 sky130_fd_sc_hd__dfrtp_1 _153_ (.CLK(clk),
    .D(cout_c),
    .RESET_B(_008_),
    .Q(net19));
 sky130_fd_sc_hd__dfrtp_1 _154_ (.CLK(clk),
    .D(net1),
    .RESET_B(_009_),
    .Q(\u_cs.lo_block.fa0.a ));
 sky130_fd_sc_hd__dfrtp_1 _155_ (.CLK(clk),
    .D(net2),
    .RESET_B(_010_),
    .Q(\u_cs.lo_block.fa1.a ));
 sky130_fd_sc_hd__dfrtp_1 _156_ (.CLK(clk),
    .D(net3),
    .RESET_B(_011_),
    .Q(\u_cs.lo_block.fa2.a ));
 sky130_fd_sc_hd__dfrtp_1 _157_ (.CLK(clk),
    .D(net4),
    .RESET_B(_012_),
    .Q(\u_cs.lo_block.fa3.a ));
 sky130_fd_sc_hd__dfrtp_1 _158_ (.CLK(clk),
    .D(net5),
    .RESET_B(_013_),
    .Q(\u_cs.hi_block_c0.fa0.a ));
 sky130_fd_sc_hd__dfrtp_1 _159_ (.CLK(clk),
    .D(net6),
    .RESET_B(_014_),
    .Q(\u_cs.hi_block_c0.fa1.a ));
 sky130_fd_sc_hd__dfrtp_1 _160_ (.CLK(clk),
    .D(net7),
    .RESET_B(_015_),
    .Q(\u_cs.hi_block_c0.fa2.a ));
 sky130_fd_sc_hd__dfrtp_1 _161_ (.CLK(clk),
    .D(net8),
    .RESET_B(_016_),
    .Q(\u_cs.hi_block_c0.fa3.a ));
 sky130_fd_sc_hd__dfrtp_1 _162_ (.CLK(clk),
    .D(net9),
    .RESET_B(_017_),
    .Q(\u_cs.lo_block.fa0.b ));
 sky130_fd_sc_hd__dfrtp_1 _163_ (.CLK(clk),
    .D(net10),
    .RESET_B(_018_),
    .Q(\u_cs.lo_block.fa1.b ));
 sky130_fd_sc_hd__dfrtp_1 _164_ (.CLK(clk),
    .D(net11),
    .RESET_B(_019_),
    .Q(\u_cs.lo_block.fa2.b ));
 sky130_fd_sc_hd__dfrtp_1 _165_ (.CLK(clk),
    .D(net12),
    .RESET_B(_020_),
    .Q(\u_cs.lo_block.fa3.b ));
 sky130_fd_sc_hd__dfrtp_1 _166_ (.CLK(clk),
    .D(net13),
    .RESET_B(_021_),
    .Q(\u_cs.hi_block_c0.fa0.b ));
 sky130_fd_sc_hd__dfrtp_1 _167_ (.CLK(clk),
    .D(net14),
    .RESET_B(_022_),
    .Q(\u_cs.hi_block_c0.fa1.b ));
 sky130_fd_sc_hd__dfrtp_1 _168_ (.CLK(clk),
    .D(net15),
    .RESET_B(_023_),
    .Q(\u_cs.hi_block_c0.fa2.b ));
 sky130_fd_sc_hd__dfrtp_1 _169_ (.CLK(clk),
    .D(net16),
    .RESET_B(_024_),
    .Q(\u_cs.hi_block_c0.fa3.b ));
 sky130_fd_sc_hd__dfrtp_1 _170_ (.CLK(clk),
    .D(net17),
    .RESET_B(_025_),
    .Q(cin_r));
 sky130_fd_sc_hd__decap_3 PHY_EDGE_ROW_0_Right_0 ();
 sky130_fd_sc_hd__decap_3 PHY_EDGE_ROW_1_Right_1 ();
 sky130_fd_sc_hd__decap_3 PHY_EDGE_ROW_2_Right_2 ();
 sky130_fd_sc_hd__decap_3 PHY_EDGE_ROW_3_Right_3 ();
 sky130_fd_sc_hd__decap_3 PHY_EDGE_ROW_4_Right_4 ();
 sky130_fd_sc_hd__decap_3 PHY_EDGE_ROW_5_Right_5 ();
 sky130_fd_sc_hd__decap_3 PHY_EDGE_ROW_6_Right_6 ();
 sky130_fd_sc_hd__decap_3 PHY_EDGE_ROW_7_Right_7 ();
 sky130_fd_sc_hd__decap_3 PHY_EDGE_ROW_8_Right_8 ();
 sky130_fd_sc_hd__decap_3 PHY_EDGE_ROW_9_Right_9 ();
 sky130_fd_sc_hd__decap_3 PHY_EDGE_ROW_10_Right_10 ();
 sky130_fd_sc_hd__decap_3 PHY_EDGE_ROW_11_Right_11 ();
 sky130_fd_sc_hd__decap_3 PHY_EDGE_ROW_12_Right_12 ();
 sky130_fd_sc_hd__decap_3 PHY_EDGE_ROW_13_Right_13 ();
 sky130_fd_sc_hd__decap_3 PHY_EDGE_ROW_14_Right_14 ();
 sky130_fd_sc_hd__decap_3 PHY_EDGE_ROW_15_Right_15 ();
 sky130_fd_sc_hd__decap_3 PHY_EDGE_ROW_16_Right_16 ();
 sky130_fd_sc_hd__decap_3 PHY_EDGE_ROW_17_Right_17 ();
 sky130_fd_sc_hd__decap_3 PHY_EDGE_ROW_0_Left_18 ();
 sky130_fd_sc_hd__decap_3 PHY_EDGE_ROW_1_Left_19 ();
 sky130_fd_sc_hd__decap_3 PHY_EDGE_ROW_2_Left_20 ();
 sky130_fd_sc_hd__decap_3 PHY_EDGE_ROW_3_Left_21 ();
 sky130_fd_sc_hd__decap_3 PHY_EDGE_ROW_4_Left_22 ();
 sky130_fd_sc_hd__decap_3 PHY_EDGE_ROW_5_Left_23 ();
 sky130_fd_sc_hd__decap_3 PHY_EDGE_ROW_6_Left_24 ();
 sky130_fd_sc_hd__decap_3 PHY_EDGE_ROW_7_Left_25 ();
 sky130_fd_sc_hd__decap_3 PHY_EDGE_ROW_8_Left_26 ();
 sky130_fd_sc_hd__decap_3 PHY_EDGE_ROW_9_Left_27 ();
 sky130_fd_sc_hd__decap_3 PHY_EDGE_ROW_10_Left_28 ();
 sky130_fd_sc_hd__decap_3 PHY_EDGE_ROW_11_Left_29 ();
 sky130_fd_sc_hd__decap_3 PHY_EDGE_ROW_12_Left_30 ();
 sky130_fd_sc_hd__decap_3 PHY_EDGE_ROW_13_Left_31 ();
 sky130_fd_sc_hd__decap_3 PHY_EDGE_ROW_14_Left_32 ();
 sky130_fd_sc_hd__decap_3 PHY_EDGE_ROW_15_Left_33 ();
 sky130_fd_sc_hd__decap_3 PHY_EDGE_ROW_16_Left_34 ();
 sky130_fd_sc_hd__decap_3 PHY_EDGE_ROW_17_Left_35 ();
 sky130_fd_sc_hd__tapvpwrvgnd_1 TAP_TAPCELL_ROW_0_36 ();
 sky130_fd_sc_hd__tapvpwrvgnd_1 TAP_TAPCELL_ROW_0_37 ();
 sky130_fd_sc_hd__tapvpwrvgnd_1 TAP_TAPCELL_ROW_0_38 ();
 sky130_fd_sc_hd__tapvpwrvgnd_1 TAP_TAPCELL_ROW_1_39 ();
 sky130_fd_sc_hd__tapvpwrvgnd_1 TAP_TAPCELL_ROW_2_40 ();
 sky130_fd_sc_hd__tapvpwrvgnd_1 TAP_TAPCELL_ROW_2_41 ();
 sky130_fd_sc_hd__tapvpwrvgnd_1 TAP_TAPCELL_ROW_3_42 ();
 sky130_fd_sc_hd__tapvpwrvgnd_1 TAP_TAPCELL_ROW_4_43 ();
 sky130_fd_sc_hd__tapvpwrvgnd_1 TAP_TAPCELL_ROW_4_44 ();
 sky130_fd_sc_hd__tapvpwrvgnd_1 TAP_TAPCELL_ROW_5_45 ();
 sky130_fd_sc_hd__tapvpwrvgnd_1 TAP_TAPCELL_ROW_6_46 ();
 sky130_fd_sc_hd__tapvpwrvgnd_1 TAP_TAPCELL_ROW_6_47 ();
 sky130_fd_sc_hd__tapvpwrvgnd_1 TAP_TAPCELL_ROW_7_48 ();
 sky130_fd_sc_hd__tapvpwrvgnd_1 TAP_TAPCELL_ROW_8_49 ();
 sky130_fd_sc_hd__tapvpwrvgnd_1 TAP_TAPCELL_ROW_8_50 ();
 sky130_fd_sc_hd__tapvpwrvgnd_1 TAP_TAPCELL_ROW_9_51 ();
 sky130_fd_sc_hd__tapvpwrvgnd_1 TAP_TAPCELL_ROW_10_52 ();
 sky130_fd_sc_hd__tapvpwrvgnd_1 TAP_TAPCELL_ROW_10_53 ();
 sky130_fd_sc_hd__tapvpwrvgnd_1 TAP_TAPCELL_ROW_11_54 ();
 sky130_fd_sc_hd__tapvpwrvgnd_1 TAP_TAPCELL_ROW_12_55 ();
 sky130_fd_sc_hd__tapvpwrvgnd_1 TAP_TAPCELL_ROW_12_56 ();
 sky130_fd_sc_hd__tapvpwrvgnd_1 TAP_TAPCELL_ROW_13_57 ();
 sky130_fd_sc_hd__tapvpwrvgnd_1 TAP_TAPCELL_ROW_14_58 ();
 sky130_fd_sc_hd__tapvpwrvgnd_1 TAP_TAPCELL_ROW_14_59 ();
 sky130_fd_sc_hd__tapvpwrvgnd_1 TAP_TAPCELL_ROW_15_60 ();
 sky130_fd_sc_hd__tapvpwrvgnd_1 TAP_TAPCELL_ROW_16_61 ();
 sky130_fd_sc_hd__tapvpwrvgnd_1 TAP_TAPCELL_ROW_16_62 ();
 sky130_fd_sc_hd__tapvpwrvgnd_1 TAP_TAPCELL_ROW_17_63 ();
 sky130_fd_sc_hd__tapvpwrvgnd_1 TAP_TAPCELL_ROW_17_64 ();
 sky130_fd_sc_hd__tapvpwrvgnd_1 TAP_TAPCELL_ROW_17_65 ();
 sky130_fd_sc_hd__clkbuf_1 input1 (.A(a[0]),
    .X(net1));
 sky130_fd_sc_hd__clkbuf_1 input2 (.A(a[1]),
    .X(net2));
 sky130_fd_sc_hd__clkbuf_1 input3 (.A(a[2]),
    .X(net3));
 sky130_fd_sc_hd__clkbuf_1 input4 (.A(a[3]),
    .X(net4));
 sky130_fd_sc_hd__clkbuf_1 input5 (.A(a[4]),
    .X(net5));
 sky130_fd_sc_hd__clkbuf_1 input6 (.A(a[5]),
    .X(net6));
 sky130_fd_sc_hd__clkbuf_1 input7 (.A(a[6]),
    .X(net7));
 sky130_fd_sc_hd__clkbuf_1 input8 (.A(a[7]),
    .X(net8));
 sky130_fd_sc_hd__clkbuf_1 input9 (.A(b[0]),
    .X(net9));
 sky130_fd_sc_hd__clkbuf_1 input10 (.A(b[1]),
    .X(net10));
 sky130_fd_sc_hd__clkbuf_1 input11 (.A(b[2]),
    .X(net11));
 sky130_fd_sc_hd__clkbuf_1 input12 (.A(b[3]),
    .X(net12));
 sky130_fd_sc_hd__clkbuf_1 input13 (.A(b[4]),
    .X(net13));
 sky130_fd_sc_hd__clkbuf_1 input14 (.A(b[5]),
    .X(net14));
 sky130_fd_sc_hd__clkbuf_1 input15 (.A(b[6]),
    .X(net15));
 sky130_fd_sc_hd__clkbuf_1 input16 (.A(b[7]),
    .X(net16));
 sky130_fd_sc_hd__clkbuf_1 input17 (.A(cin),
    .X(net17));
 sky130_fd_sc_hd__clkbuf_1 input18 (.A(reset),
    .X(net18));
 sky130_fd_sc_hd__buf_2 output19 (.A(net19),
    .X(cout));
 sky130_fd_sc_hd__buf_2 output20 (.A(net20),
    .X(sum[0]));
 sky130_fd_sc_hd__buf_2 output21 (.A(net21),
    .X(sum[1]));
 sky130_fd_sc_hd__buf_2 output22 (.A(net22),
    .X(sum[2]));
 sky130_fd_sc_hd__buf_2 output23 (.A(net23),
    .X(sum[3]));
 sky130_fd_sc_hd__buf_2 output24 (.A(net24),
    .X(sum[4]));
 sky130_fd_sc_hd__buf_2 output25 (.A(net25),
    .X(sum[5]));
 sky130_fd_sc_hd__buf_2 output26 (.A(net26),
    .X(sum[6]));
 sky130_fd_sc_hd__buf_2 output27 (.A(net27),
    .X(sum[7]));
 sky130_fd_sc_hd__buf_4 fanout28 (.A(net30),
    .X(net28));
 sky130_fd_sc_hd__buf_4 fanout29 (.A(net30),
    .X(net29));
 sky130_fd_sc_hd__clkbuf_4 fanout30 (.A(net18),
    .X(net30));
endmodule
